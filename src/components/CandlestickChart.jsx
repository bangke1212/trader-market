...
export default function CandlestickChart({ symbol, timeframe }) {
  ...
  const draw = (ctx, w, h, data, p) => {
    ...
    const ys = ch / (max - min)
    const tx = (i) => pad.left + (i / (data.length - 1 || 1)) * cw
    const ty = (v) => pad.top + (max - v) * ys

    // Grid
    ctx.strokeStyle = '#1a1a22'; ctx.lineWidth = 0.5
    for (let i = 0; i < 5; i++) {
      const y = pad.top + (ch / 4) * i, val = max - (range / 4) * i
      ctx.beginPath(); ctx.moveTo(pad.left, y); ctx.lineTo(w - pad.right, y); ctx.stroke()
      ctx.fillStyle = '#666'; ctx.font = '10px sans-serif'; ctx.fillText('$'+val.toFixed(1), w-pad.right+4, y+3)
    }

    // Candles
    const bw = Math.max(1.5, cw / data.length * 0.7)
    for (let i = 0; i < data.length; i++) {
      const d = data[i], x = tx(i), up = d.close >= d.open
      ctx.strokeStyle = up ? '#00c853' : '#ff1744'; ctx.fillStyle = up ? '#00c853' : '#ff1744'
      ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(x, ty(d.high)); ctx.lineTo(x, ty(d.low)); ctx.stroke()
      const bodyH = Math.max(1, Math.abs(d.open - d.close) * ys)
      ctx.fillRect(x - bw/2, ty(Math.max(d.open, d.close)), bw, bodyH)
    }
    ...
    if (p && data.length) {
      ...
      ctx.fillStyle = '#ddd'; ctx.font = 'bold 11px sans-serif'; ctx.fillText('$'+p.toFixed(1), w-pad.right+4, py+4)
    }
  }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return
        if (!klines?.length) { setError('No data from CoinGecko'); setLoading(false); return }
        dataRef.current = klines
        setPrice(klines[klines.length-1].close)
        setLoading(false)
        unsub = subscribeToKlines(symbol, timeframe, (u) => {
          if (cancelled) return
          const last = dataRef.current[dataRef.current.length-1]
          if (last && u.price) {
            // Update last candle with latest price
            last.close = u.price
            if (u.price > last.high) last.high = u.price
            if (u.price < last.low) last.low = u.price
          }
          setPrice(u.price || price)
        })
      } catch (e) {
        if (cancelled) return
        setError(`Failed: ${e.message} — CoinGecko may be rate-limited. Try again.`)
        setLoading(false)
      }
    })()
      ...
      {error && !loading && (
        <div className="absolute inset-0 z-10 flex items-center justify-center bg-[#0d0e12]/90">
          <div className="flex flex-col items-center gap-3 px-6 text-center max-w-sm">
            ...
            <button onClick={() => window.location.reload()} className="mt-1 px-4 py-1.5 bg-slate-700 hover:bg-slate-600 text-slate-200 rounded-lg text-xs">
              Reload
            </button>
          </div>
        </div>
